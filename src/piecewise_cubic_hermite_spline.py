...
def pchip_coeff_generator(x, y, sort_x=False, derives="2"):
...
def pchip_1d_generator(x, y, tar_xs, plotting=0, derives="2"):

    plt_x = []
    plt_y = []
    sorted(x)
    sorted(y)
    coeff = pchip_coeff_generator(x, y, derives=derives)
    # print(coeff)
