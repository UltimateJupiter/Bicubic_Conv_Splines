...
def error_analysis(start, end, func, interp_func, h, eval_number):
    ...
    ret_y = interp_func(x, y, target_x)
...
def error_convergence(interpolation_func, start, end, h_seq, test_funcs, test_names, name):
    ...
    for h in h_seq:
        ...
        for i in range(len(test_funcs) - 1):
            err, d = error_analysis(start, end, test_funcs[i], interpolation_func, h, 1000)
            err_ls[i].append(err)
...
def visualization_plot():
    ...
    pchip_1d = piecewise_cubic_hermite_spline.pchip_1d_generator(x, y, tar_x)
    ...
    plt.legend(["data", "pchip", "natural cubic"])
    plt.show()
...
def convergence_analysis():
    ...
    error_convergence(piecewise_cubic_hermite_spline.pchip_1d_generator, -1, 1, h_sequence_spline, test_funcs_pchip, test_names_pchip, "Pchip")
# visualization_plot()
